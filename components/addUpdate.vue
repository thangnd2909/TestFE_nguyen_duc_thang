    ...
    <v-card>
        ...
        <v-container class="pb-6 pt-5" fluid>
          <v-row>
            ...
            <v-col cols="12" md="6" class="py-0 ">
              ...
              <v-select
                v-model="bloodGroup"
                :items="listBloodGroup"
                outlined
                dense
                :error-messages="bloodGroupErrors"
                @input="bloodGroupErrors = []"
              ></v-select>
            </v-col>
            ...
            <v-col cols="12" md="6" class="py-0 ">
              ...
              >
                <template v-slot:activator="{ on }">
                  <v-text-field
                    v-on="on"
                    class="color-icon-time fs-15px pb-error-0"
                    v-model="birthday"
                    dense
                    outlined
                    readonly
      ...
      <v-card-actions>
        ...
        <v-btn
          text
          height="40px"
          class="orange lighten-2 text-none white--text"
          @click="checkAddUpdate"
        >
        ...
        <v-btn
          text
          height="40px"
          class="red darken-1 lighten-4 text-none white--text"
          @click="toggle()"
        >
  ...
  methods: {
    ...
    checkAddUpdate() {
      ...
      if (this.$isNullOrEmpty(this.detailItem)) {
        if (!this.$isNullOrEmpty(this.phone)) {
          for (
            let i = 0;
            i < JSON.parse(localStorage.getItem('ListDataItem')).length;
            i++
          ) {
            if (
              this.phone ===
              JSON.parse(localStorage.getItem('ListDataItem'))[i].phone
            ) {
              hasError = true
          ...
          for (
            ...
            if (
              this.phone ===
                JSON.parse(localStorage.getItem('ListDataItem'))[i].phone &&
              this.detailItem.id !==
                JSON.parse(localStorage.getItem('ListDataItem'))[i].id
            ) {
              hasError = true
              this.phoneErrors = ['SĐT đã tồn tại trong hệ thống']
            }
      ...
      if (this.$isNullOrEmpty(this.name)) {
        ...
        hasError = true
        this.nameErrors = [
          'Vui lòng nhập họ và tên không được vượt quá 50 kí tự'
        ]
      }
      ...
      if ((this.address || '').trim().length > 255) {
        hasError = true
        this.addressErrors = [
          'Vui lòng nhập địa chỉ không được vượt quá 255 kí tự'
        ]
      }
      if (!hasError) {
        if (this.$isNullOrEmpty(this.detailItem)) {
    ...
    update() {
      const data = {
        ...
        full_name: !this.$isNullOrEmpty(this.name)
          ? this.name.trim()
          : this.name,
        phone: this.phone,
        birthday: this.birthday,
        email: !this.$isNullOrEmpty(this.email)
          ? this.email.trim()
          : this.email,
        gender: this.sex,
        address: !this.$isNullOrEmpty(this.address)
          ? this.address.trim()
          : this.address,
      ...
      this.$store.dispatch('notification/set_notifications', {
        ...
        timeout: 5000,
        color: 'success',
        text: `Hồ sơ ${this.name} - ${this.phone} được cập nhật thành công`
      })
      this.reset()
    },
    Add() {
      ...
      listItemNew.push(
        data,
        ...JSON.parse(localStorage.getItem('ListDataItem'))
      )
      ...
      this.$store.dispatch('notification/set_notifications', {
        ...
        timeout: 5000,
        color: 'success',
        text: `Hồ sơ ${this.name} - ${this.phone} được thêm mới thành công`
      })
    ...
    open(value) {
      if (value) {
        ...
        if (!this.$isNullOrEmpty(this.detailItem)) {
          ...
          this.menu_date = moment(
            this.detailItem.birthday,
            'DD/MM/YYYY'
          ).format('YYYY-MM-DD')
        }
      }
